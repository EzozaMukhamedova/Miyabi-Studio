...
function HeaderActionButtons() {
  ...
  const buttons = [
    ...
    { id: 3, title: t("cart"), icon: <CartIcon />, count: 7, to: "/cart" },
    {
      id: 4,
      title: t("profile"),
      icon: <UserIcon />,
  ...
  return (
    <>
      ...
      <div
        className={`fixed max-w-[300px] w-full h-screen bg-white shadow-2xl z-[51] top-0 ${
      ...
      >
        ...
        <div
          className={`flex flex-col justify-start items-center gap-[10px] px-5 mt-5`}
        >
          {buttons?.map((item) => (
            <HeaderActions
              key={item.id}
              ...
              onClick={item.onClick}
              extraStyle={
                "!w-full !justify-start !px-5 hover:bg-[#15509E]/30 duration-300"
              }
            />
          ))}
        </div>
